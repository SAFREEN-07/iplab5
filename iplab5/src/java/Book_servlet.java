...
public class Book_servlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
       response.setContentType("text/html;charset=UTF-8");
        book=new Book("Dr.A.P.J.Abdul Kalaam","My Journey",1000.00,"GG Publisher",40);
        map.put(book.title, book);
        
        book=new Book("Dr.Bibek Debroy","Making of New India",750.00,"TR Publisher",23);
        map.put(book.title, book);
        
        book=new Book("Dr.Krishna Saksena","Whispers of Time",870.50,"MJ Publisher",26);
        map.put(book.title, book);
         ...
         try (PrintWriter out = response.getWriter()) {
            ...
            if(map.containsKey(bName))
            {
                ...
                if(book.author.equals(author))
                {
                    ...
                    out.println("<form method='get' action='http://localhost:8082/IP_LAB_EXERCISES/Book_servlet'>");
